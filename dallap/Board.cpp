...
pair< vector< vector< int > > , vector< vector< int > > > Board::generate_remove_markers(int player, int row1, int position1, int row2, int position2)
{
    ...
    // cerr << "Inside Generate Remove Markes\n";
    // print_board();
    // cerr << " Player: " << player << ", row1: " << row1 << ", position1" << position1 << ", row2: " << row2 << ", position2: " << position2 << "\n";
    ...
    if(row1 == row2)
    {
        ...
        // if(remove_markers_player.size() != 0 || remove_markers_opponent.size() != 0)
        // {
        //     cerr << "In Case1: \n";
        //     cerr << "For Player: \n";
        //     for(int i = 0; i < remove_markers_player.size(); i++)
        //         cerr << remove_markers_player[i][0] << " " << remove_markers_player[i][1] << " " << remove_markers_player[i][2] << " " << remove_markers_player[i][3] << "||||  ";
        //     cerr << "For Opponent\n";
        //     for(int i = 0; i < remove_markers_opponent.size(); i++)
        //         cerr << remove_markers_opponent[i][0] << " " << remove_markers_opponent[i][1] << " " << remove_markers_opponent[i][2] << " " << remove_markers_opponent[i][3] << "||||  ";
        //     cerr << "Exiting Generate Valid Markers\n";
        // }
    ...
    else if(position1 == position2)
    {
        ...
        for(int itr = row1; (itr - row2) * c < 0; itr+=c)
        {
            if(board[itr][position1] != 0)
            {
                ...
                low = range.first;
                high = range.second;
                for(int i = position + 1; ((i <= high) && (board[row][i] == color)); i++)
                    count_front++;
                for(int i = position - 1; ((i >= low) && (board[row][i] == color)); i--)
                    count_back++;
                if(count_front + count_back + 1 >= num_resolution)
                {
...
vector< vector< vector< int > > > Board::unique_non_intersecting_remove_markers(vector< vector< int > > remove_markers_player)
{
    ...
    // {
    //     cerr << "Inside Unique non intersecting function\n";
    //     for(int i = 0; i < remove_markers_player.size(); i++)
    //         cerr << remove_markers_player[i][0] << " " << remove_markers_player[i][1] << " " << remove_markers_player[i][2] << " " << remove_markers_player[i][3] << "\n";
    // }
    vector < vector< vector< int > > > answer;
    vector < vector < vector < int > > > answer_else;
    if(remove_markers_player.size() == 0)
        return answer;
    else
    {
        ...
        vector< vector < vector < int > > > answer1 = unique_non_intersecting_remove_markers(remove_markers_player);
        for(int i = 0; i < remove_markers_player.size(); i++)
        {
        ...
        for(int i = 0; i < answer1.size(); i++)
        {
            ...
            for(int j = 0; j < answer1[i].size(); j++)
                if(intersect(removed_0[0], removed_0[1], removed_0[2], removed_0[3], answer1[i][j][0], answer1[i][j][1], answer1[i][j][2], answer1[i][j][3]))
                {
                    check = false;
                    break;
                }
        ...
        answer_else = unique_non_intersecting_remove_markers(non_intersecting_remove_markers);
        if(answer_else.size() == 0)
        {
            vector< vector < int > > single;
            single.push_back(removed_0);
            answer_else.push_back(single);
        }
        else
        {
            for(int i = 0; i < answer_else.size(); i++)
                answer_else[i].push_back(removed_0);
        }
    }
    answer.insert(answer.end(), answer_else.begin(), answer_else.end());
    // if(answer.size() != 0){
    //   cerr << "Answer from Unique Non Intersecting Rows found: \n";
    //   for(int i = 0; i < answer.size(); i++)
    //   {
    //       for(int j = 0; j < answer[i].size(); j++)
    //           cerr << answer[i][j][0] << " " << answer[i][j][1] << " " << answer[i][j][2] << " " << answer[i][j][3] << ",    ";
    //       cerr << "\n";
    //   }
    //   cout << "Exiting Unique non-intersecting Rows\n";
    // }
    return answer;
}
